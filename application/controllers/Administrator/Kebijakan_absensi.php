...
class kebijakan_absensi extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['tanggal'] = $this->Db_select->select_all_where('tb_event', 'date_format(tanggal_event,"%Y") = date_format(now(),"%Y") And id_channel=' . $id_channel);
		// echo json_encode($data['data']);exit();
		$this->load->view('Administrator/header', $data);
		$this->load->view('Administrator/kebijakan');
	...
	public function insert_event()
	{
		...
		for ($i = 0; $i < ($interval + 1); $i++) {
			$tanggal_event = date('Y-m-d', strtotime($this->input->post('start_date') . " +" . $i . " days"));
	...
	{
		$where['id_event'] = $this->input->post('id_event');
		$delete = $this->Db_dml->update('tb_event', ['is_deleted' => 1], $where);
		if ($delete == 1) {
			$result['status'] = true;
			$result['message'] = 'Data berhasil disimpan.';
			$result['data'] = array();
